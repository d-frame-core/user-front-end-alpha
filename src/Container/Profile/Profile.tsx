...
function Profile() {
  ...
  const verifyOtp = (e: any) => {
    ...
    (window as any).confirmationResult.confirm(otp).then((result: any) => {
  ...
  return (
    <div>
      ...
      <Box className='profilebox' sx={{ display: "flex" }}>
        <Container maxWidth={false} sx={{ maxWidth: '85%', display: 'flex' }} className='prbox1'>
          ...
          <Container maxWidth={false} sx={{ maxWidth: '60%', display: 'flex' }} className='contitem'>
            ...
            <div><a className='pr'>Number</a><a className='colon1'>:</a><a className='prfont'>{phonenumber} <a onClick={() => setShowPopup(true)}><CreateOutlinedIcon sx={{ color: '#47B5FF', top: '4px', left: '15%', position: 'relative' }} /></a></a></div>
      ...
      {showPopup && <Backdrop open={showPopup} sx={{ zIndex: (theme) => theme.zIndex.drawer + 1 }}>
        <Box className='update'>
          ...
          <div></div>
          <CloseIcon onClick={() => setShowPopup(false)} className='cross' />
        </Box>
      </Backdrop>
      }
    </div>
